...
nix-shell -p git --command "git clone https://github.com/StevenBuglione/nixos-config.git $SCRIPT_DIR"
...
sed -i "0,/sbuglione/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/StevenBuglione/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/stevenbuglione1@gmail.com//" $SCRIPT_DIR/flake.nix
